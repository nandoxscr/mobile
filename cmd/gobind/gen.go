...
import (
	...
	"github.com/nandoxscr/mobile/bind"
	"github.com/nandoxscr/mobile/internal/importers"
	"github.com/nandoxscr/mobile/internal/importers/java"
	"github.com/nandoxscr/mobile/internal/importers/objc"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/nandoxscr/mobile/bind")
			if err != nil {
				errorf(`"github.com/nandoxscr/mobile/bind" is not found; run go get github.com/nandoxscr/mobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/nandoxscr/mobile directory.
			...
			javaDir, err := packageDir("github.com/nandoxscr/mobile/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/nandoxscr/mobile/bind/objc")
			if err != nil {
