...
func TestBindAndroid(t *testing.T) {
	...
	for _, tc := range tests {
		...
		cmdBind.flag.Parse([]string{"github.com/nandoxscr/mobile/asset"})
		err := runBind(cmdBind)
		if err != nil {
			t.Log(buf.String())
			t.Fatal(err)
...
WORK=$WORK
GOOS=android CGO_ENABLED=1 gobind -lang=go,java -outdir=$WORK{{if .JavaPkg}} -javapkg={{.JavaPkg}}{{end}} github.com/nandoxscr/mobile/asset
...
GOOS=ios CGO_ENABLED=1 gobind -lang=go,objc -outdir=$WORK/ios -tags=ios{{if .Prefix}} -prefix={{.Prefix}}{{end}} github.com/nandoxscr/mobile/asset
...
require github.com/nandoxscr/mobile v0.0.0-20230905140555-fbe1c053b6a9
...
github.com/nandoxscr/mobile v0.0.0-20230905140555-fbe1c053b6a9 h1:LaLfQUz4L1tfuOlrtEouZLZ0qHDwKn87E1NKoiudP/o=
github.com/nandoxscr/mobile v0.0.0-20230905140555-fbe1c053b6a9/go.mod h1:2jxcxt/JNJik+N+QcB8q308+SyrE3bu43+sGZDmJ02M=
...
import (
	_ "github.com/nandoxscr/mobile/app"
)
...
func TestBindWithGoModules(t *testing.T) {
	...
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/nandoxscr/mobile/cmd/gobind").CombinedOutput(); err != nil {
		t.Fatalf("%v: %s", err, string(out))
	}
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/nandoxscr/mobile/cmd/gomobile").CombinedOutput(); err != nil {
		t.Fatalf("%v: %s", err, string(out))
	}
	path := dir
	...
	for _, target := range []string{"android", "ios"} {
		...
		t.Run(target, func(t *testing.T) {
			...
			tests := []struct {
				...
				{
					Name: "Absolute Path",
					Path: "github.com/nandoxscr/mobile/bind/testdata/cgopkg",
				},
				{
					Name: "Relative Path",
